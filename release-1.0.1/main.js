...
var MarkwhenSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  addBasicSettings(containerEl) {
    ...
    new import_obsidian.Setting(containerEl).setName("Notes folder path").setDesc("Path to your notes folder").addText((text) => text.setPlaceholder("notes").setValue(this.plugin.settings.notesFolderPath).onChange(async (value) => {
  ...
  addFilterSettings(containerEl) {
    ...
    new import_obsidian.Setting(containerEl).setName("Excluded folders").setDesc("Subfolders to exclude (comma-separated, relative to Notes Folder Path)").addText((text) => text.setPlaceholder("archive, temp").setValue(this.plugin.settings.filterConfig.excludeFolders.join(", ")).onChange(async (value) => {
...
var MarkwhenSync = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on("create", (file) => {
        if (this.settings.enableAutoSync && file instanceof import_obsidian.TFile && file.extension === "md" && file.path.startsWith(this.settings.notesFolderPath + "/")) {
    ...
    this.registerEvent(
      this.app.vault.on("delete", (file) => {
        if (this.settings.enableAutoSync) {
          if (file instanceof import_obsidian.TFile && file.path.startsWith(this.settings.notesFolderPath + "/")) {
  ...
  async collectEntries() {
    ...
    for (const file of files) {
      ...
      const relativePath = file.path.substring(notesFolder.path.length + 1);
  ...
  async updateNoteFromEvent(event) {
    ...
    if (!event.startDate || !event.endDate || !window.moment(event.startDate, "YYYY-MM-DD", true).isValid() || !window.moment(event.endDate, "YYYY-MM-DD", true).isValid()) {
